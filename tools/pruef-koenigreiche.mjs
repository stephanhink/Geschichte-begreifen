// Wegwerf-Prüfskript für die Königreiche-Karte (nicht Teil von npm test).
import { createRequire } from 'node:module';
const require = createRequire(import.meta.url);
const karte = require('../utils/themen/karten/koenigreiche.js');
...
const landmarken = [
  ...
  ['Cork, Suedkueste Irlands', -8.3, 51.85],
  ...
  ['Varna am Schwarzen Meer', 27.92, 43.2],
...
for (const [n, lon, lat] of landmarken) {
  ...
  console.log((a < TOL ? '  ok  ' : ' FEHL ') + n.padEnd(28) + a.toFixed(1));
}
...
for (const [n, lon, lat] of abseits) {
  const a = abstand(lon, lat);
  console.log((a > TOL * 2 ? '  ok  ' : ' FEHL ') + n.padEnd(28) + a.toFixed(1));
}
